...
1. **Source Code Audit**: Use `local-paper-navigator`'s `find_code.py` (online) and `code_repo_search.py` (local code-repo) to check whether baseline source code is available. Classify mode as Adapt (code exists), From-Scratch (no code), or Hybrid (partial). Record in `assets/source-code-audit-template.md` → `/experiments/stage1_baseline/source-code-audit.md`

2. **Adapt mode**: Follow standard 5-step process (find code → run → match config → compare metrics → diagnose gap):
...
→ Write audit to `/experiments/stage1_baseline/source-code-audit.md`
