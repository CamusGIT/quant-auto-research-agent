...
Scripts at `skills/quant-paper-extractor/scripts/`. Run via `python3 scripts/<name>.py`.
...
Resolve the Python interpreter once and reuse:
...
```bash
"$PYTHON_BIN" skills/quant-paper-extractor/scripts/pdf_to_markdown.py \
...
```bash
"$PYTHON_BIN" skills/quant-paper-extractor/scripts/manifest.py list \
...
```bash
"$PYTHON_BIN" skills/quant-paper-extractor/scripts/extract.py \
  --prepare --markdown-file markdown/{paperId}.md
```
...
- **Pass 1** (first ~8,000 chars): Extract `paperId`, `title`, `year`, `broker`, `tldr`, `abstract`, `keywords`
...
| title | str | ≤30 | English title of the report |
| year | int | 4 digits | Publication year |
| broker | str | ≤10 | English name of securities firm |
...
```bash
"$PYTHON_BIN" skills/quant-paper-extractor/scripts/extract.py \
  --validate --record wiki/{paperId}.jsonl
```
...
```bash
"$PYTHON_BIN" skills/quant-paper-extractor/scripts/manifest.py update is not needed — \
  use extract.py --validate which updates manifest status automatically
```

Or manually update by editing `manifest.jsonl` to set status=`extraction_done` for the processed paperId.
...
```bash
"$PYTHON_BIN" skills/quant-paper-extractor/scripts/extract.py \
  --validate --wiki-dir wiki/ --manifest-path manifest.jsonl
```
