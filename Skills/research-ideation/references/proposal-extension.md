...
Use `local-paper-navigator`'s `find_code.py` (online) and `code_repo_search.py` (local code-repo) to check each baseline's source code availability, then fill:

| Baseline | Source Code? | Impl Mode | LOC Est | Difficulty Adj |
|----------|-------------|-----------|---------|---------------|
| B1: [method] | ✅ Official / ✅ Community (≥50 stars) / ❌ None | Adapt/From-Scratch/Hybrid | [realistic LOC] | +0/+3-5/+1-2 |
| B2: [method] | ... | ... | ... | ... |

**Implementation Mode classification**:
